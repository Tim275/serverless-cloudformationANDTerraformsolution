...
```bash
uebung1601$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
uebung1601$ sam local start-api
uebung1601$ curl http://localhost:3000/
```
...
```bash
uebung1601$ sam logs -n HelloWorldFunction --stack-name uebung1601 --tail
```
...
```bash
sam delete --stack-name uebung1601
```
